...
class Simulation(object):
    ...
    def save_fields_to_vtk(self, filename):
        """ Checks if an index monitor with the name "global_fields" exists. If so, it will store the 'Hz' attribute in a .vtr file.
            
            Parameters
            ----------
            :param filename:    filename of the VTK file to store the index data
        """
        if self.fdtd.getnamednumber("global_fields") > 0:
            script=('globalHz = getresult("global_fields", "H");'
                    'vtksave("{}.vtr", globalHz);'
                    'clear(globalHz);').format(filename)
            self.fdtd.eval(script)
