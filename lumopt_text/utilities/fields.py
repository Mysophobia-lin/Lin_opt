...
class Fields(object):
    ...
    def scale(self, dimension, factors):
        ...
        """
        ## For performance reasons, we will use broadcasting. So, expand the factors to be of the same shape as the fields (5d) but all singular except for
        ## the one that we wish to scale
        new_shape = [1,1,1,1,1]
        new_shape[dimension] = len(factors)
        reshaped_factors = np.reshape(factors,new_shape)

        if hasattr(self.E, 'dtype'):
            if self.E.shape[dimension] == len(factors):
                self.E *= reshaped_factors
                self.getfield = self.make_field_interpolation_object(self.E)
            else:
    ...
    def plot(self,ax,title,cmap):
        ...
        ax.pcolormesh(xx*1e6, yy*1e6, np.abs(Ex**2+Ey**2+Ez**2) ,cmap=plt.get_cmap(cmap), shading='auto')
    ...
    def plot_field(self,field_func=None,original_grid=True,wl=1550e-9,name='field'):
        ...
        if len(self.x) > 1 and len(self.y) > 1:
            ax1.pcolormesh(xx*1e6, yy*1e6, np.real(Ex), cmap=plt.get_cmap('bwr'), shading='auto')
            ax1.set_title('real('+name+'x)')
            ax2.pcolormesh(xx*1e6, yy*1e6, np.real(Ey), cmap=plt.get_cmap('bwr'), shading='auto')
            ax2.set_title('real('+name+'y)')
            ax3.pcolormesh(xx*1e6, yy*1e6, np.real(Ez), cmap=plt.get_cmap('bwr'), shading='auto')
