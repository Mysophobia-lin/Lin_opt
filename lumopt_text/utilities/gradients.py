...
class GradientFields(object):
    ...
    def plot_gradients(self, fig, ax_gradients, original_grid):
        ...
        im = ax_gradients.pcolormesh(xx*1e6, yy*1e6, Sparse_pert, vmin=-max_val, vmax=max_val, cmap = plt.get_cmap('bwr'), shading='auto')
    ...
    def plot_eps(self,ax_eps):
        ...
        im = ax_eps.pcolormesh(xx*1e6, yy*1e6, np.real(np.transpose(eps)), shading='auto')#, cmap=plt.get_cmap('bwr'))
    ...
    def spatial_gradient_integral_on_cad(sim, forward_fields, adjoint_fields, wl_scaling_factor):
        ...
        sim.fdtd.eval("clear(param_idx, wl_idx, num_opt_params, num_wl_pts, spatial_integrand, gradient_fields, wl_scaling_factor, partial_fom_derivs_vs_lambda, d_epses);")
