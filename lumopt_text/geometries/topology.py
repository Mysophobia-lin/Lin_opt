...
class TopologyOptimization2DParameters(Geometry):
    ...
    def get_params_from_eps(self,eps):
        return self.calc_params_from_eps(eps)

    def set_params_from_eps(self,eps):
        self.last_params = self.get_params_from_eps(eps)

    def check_param_count_matches_bounds(self, param):
        if np.size(param) != len(self.bounds):
            raise UserWarning("Project file structure 'initial_guess' reports "+str(np.size(param))+" parameters where expected "+str(len(self.bounds)) +
            ". This can occur when the number of x, y coordinates specificed to LumOpt do not match the dimensions of the FDTD grid. Try using similar " +
            "or divisible delta values (size of a pixel) across all the axes.")

    def check_param_count_matches_sim_eps(self, params, sim):
        fields_eps_size = np.size(self.get_params_from_eps(get_eps_from_sim(sim.fdtd, unfold_symmetry=False)[0])) #< Don't unfold when getting the inital parameters
        if fields_eps_size != 1 and fields_eps_size != np.size(params):
            raise UserWarning("Project file index monitor reports "+str(fields_eps_size)+" coordinates, where expected "+str(np.size(params)) + 
            ". This can occur when the initial condition specified to LumOpt do not match the dimensions of the FDTD grid. Try using similar " +
            "or divisible delta values (size of a pixel) across all the axes.")
    ...
    def extract_parameters_from_simulation(self, sim):
        ...
        self.check_param_count_matches_bounds(self.last_params)
...
class TopologyOptimization2D(TopologyOptimization2DParameters):
    ...
    def get_params_from_eps(self,eps):
        ...
        return super().get_params_from_eps(eps[:,:,0,0,2])
    ...
    def add_geo(self, sim, params=None, only_update = False):
        ...
        if not only_update:
            ...
            script=('setnamed("FDTD","min mesh step", {});').format(min(self.dx, self.dy))
            fdtd.eval(script)
        ...
        if params is not None:
            self.check_param_count_matches_sim_eps(params, sim)
...
class TopologyOptimization3DLayered(TopologyOptimization2DParameters):
    ...
    def get_params_from_eps(self,eps):
        ...
        super().get_params_from_eps(eps[:,:,midZ_idx,0,2])
    ...
    def add_geo(self, sim, params=None, only_update = False):
        ...
        if not only_update:
            ...
            script=('setnamed("FDTD","min mesh step", {});').format(min(self.dx, self.dy, self.dz))
            fdtd.eval(script)
