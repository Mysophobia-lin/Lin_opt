...
class ModeMatch(object):
    ...
    @staticmethod
    def add_mode_expansion_monitor(sim, monitor_name, mode_expansion_monitor_name, mode, rotation_angle_theta, rotation_angle_phi, rotation_offset):
        ...
        if sim.fdtd.getnamednumber(mode_expansion_monitor_name) == 0:
            ...
            sim.fdtd.setnamed(mode_expansion_monitor_name, 'theta', rotation_angle_theta)
            if sim.fdtd.getnamednumber('FDTD') == 1:
                if sim.fdtd.getnamed('FDTD', 'dimension') == '3D':  
                    sim.fdtd.setnamed(mode_expansion_monitor_name, 'phi', rotation_angle_phi)
            sim.fdtd.setnamed(mode_expansion_monitor_name, 'rotation offset', rotation_offset)
    ...
    def get_wavelengths(sim):
        if sim.fdtd.getglobalmonitor('sample spacing') == 'uniform':
            ...
            return Wavelengths(start = (sp.constants.speed_of_light/(sim.fdtd.getglobalmonitor('custom frequency samples'))).flatten()).asarray()
