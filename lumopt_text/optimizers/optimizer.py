...
class Optimizer(object):
    ...
    def __init__(self, max_iter, scaling_factor = None, scale_initial_gradient_to = 0, penalty_fun = None, penalty_jac = None, logging_path = None):
        ...
        self.penalty_jac_approx = Optimizer.create_jac_approx(self.penalty_fun)
...
#        assert bounds.shape[0] == start_params.size and bounds.shape[1] == 2
        if self.scaling_factor is not None:
            ...
            new_scaling_factor = np.ones(len(start_params))
            new_offset = np.zeros(len(start_params))
        ...
        if self.bounds is not None:
            ...
            for i in range(len(self.bounds)):
                bounds_diff = bounds[i][1] - bounds[i][0]
                if bounds_diff <= 0.0:
                    raise UserWarning("bound ranges must be positive.")
                elif self.scaling_factor is None:
                    new_scaling_factor[i] = 1/bounds_diff
                    new_offset[i] = bounds[i][0]
            if self.scaling_factor is None:
                self.scaling_factor = np.array(new_scaling_factor)
                self.scaling_offset =  np.array(new_offset)
                self.bounds = ([(0,1), ] * len(self.scaling_factor))
            else:
    ...
    def reset_start_params(self, start_params, scale_initial_gradient_to):
        ...
        self.start_point = (start_params-self.scaling_offset) * self.scaling_factor 
    ...
    def report_writing(self):
        with open(self.logfile,'a') as f:
            ...
            f.write('PARAMETERS = {}\n'.format(np.array2string(self.params_hist[-1] / self.scaling_factor + self.scaling_offset , separator = ', ', max_line_width = 10000)))
            f.write('\n \n')
    ...
    def create_jac_approx(fom_func):
        def finite_diff_approx(opt_params):
            ...
            for index, param in np.ndenumerate(opt_params):
                ...
                jac[index] = sp.misc.derivative(func = single_param_fom_func, x0 = param, dx = 1.0e-5, n = 1, order = 3)
            return jac
